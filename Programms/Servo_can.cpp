...
int get_id(int32_t data)
{
  return (data - (data%1000))/1000;
}
int calc(int32_t data )
{
  int angle = data -(data - (data%1000));
  return map(angle, 0, 180, 125, 575);
}
void loop() {
  ...
  while(HAL_FDCAN_GetRxFifoFillLevel(hfdcan1, FDCAN_RX_FIFO0) > 0 )
    {
      ...
      // Serial.println("  ");
      int32_t convert_data =  (RxData[3] << 24) | (RxData[2] << 16) | 
                              (RxData[1] << 8) | RxData[0];
      // Serial.println(convert_data);
      // Serial.println(get_id(convert_data));
      // Serial.println(calc(convert_data));
      pwm.setPWM(get_id(convert_data), 0, calc(convert_data));
